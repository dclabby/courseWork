...
/* Write a query to return the lowest 20 orders in terms of smallest total_amt_usd. Include the id*/
SELECT id, account_id, total_amt_usd
FROM orders
...
standard_amt_usd/standard_qty AS cost_per_qty
FROM orders
LIMIT 10;
...
SELECT id, account_id,
100*poster_amt_usd/(standard_amt_usd + gloss_amt_usd + poster_amt_usd) AS percent_revenue
FROM orders
LIMIT 10;
...
question by writing a query that displays the order date and gloss_qty data for all orders where gloss_qty is between 24 and 29. Then look at your 
output to see if the BETWEEN operator included the begin and end values or not.*/
SELECT occurred_at, gloss_qty
FROM orders
WHERE gloss_qty BETWEEN 24 AND 29;
/*... it does include limit values, i.e. 24 & 29*/

/*Use the web_events table to find all information regarding individuals who were contacted via the organic or adwords channels, and started their 
account at any point in 2016, sorted from newest to oldest.*/
SELECT *
FROM web_events
WHERE channel IN ('organic', 'adwords')
AND occurred_at BETWEEN '2016-01-01' AND '2017-01-01' /*Note - end point assumes midnight when only date specified, i.e. 2016-12-31T23:59 + 1 minute*/
